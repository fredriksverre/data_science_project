...
ui <- fluidPage(
  ...
  sidebarLayout( 
    sidebarPanel( 
      sliderInput("df", "Dates:",min = as.Date("2016-01-01","%Y-%m-%d"), max = as.Date(Sys.time(),"%Y-%m-%d"),value=c(as.Date("2018-01-01"),as.Date("2020-01-01")),timeFormat="%Y-%m-%d")),mainPanel(plotOutput("plot2")))) 
...
server <- function(input,output){
  ...
  output$plot2<-renderPlot({
    ...
    ggplot(data=dat()%>%
      ...
      ylab(expression("Change on cumulative form"))+
      xlab("Date")},height = 300,width = 500)}
shinyApp(ui, server)
