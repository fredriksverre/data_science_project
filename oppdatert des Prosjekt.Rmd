...
```{r message=FALSE, echo=FALSE}
ETH2 <- ETH %>% rename( PriceETH = "Price", week2 = "week")
BTC2 <- BTC %>% rename( PriceBTC = "Price", Asset2  = "Asset")

df2<-cbind(ETH2,BTC2)

rolling_corr$corr <- correlate(ETH2$PriceETH,BTC2$PriceBTC)
...
rolling_corr %>%
  ...
  # Data
  #geom_line(aes(y = corr.x), color = "red") +
